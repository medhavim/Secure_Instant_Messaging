...
class Client(cmd.Cmd):
	...
	while login_attempts < MAX_LOGIN_ATTEMPTS and not loggedIn:
	    loggedIn, userName = self.getCredentials()
	    login_attempts += 1
	    if loggedIn:
		self.userName = userName
		client.prompt = self.userName + CMD_PROMPT
		client.cmdloop('###### User <' + userName + '> successfully login')
	if not loggedIn:
	...
	try:
	    ...
	    print 'Step 1: initiate the authentication to server'
	    auth_init_response = self._auth_init()
	    solved_challenge = self._handle_auth_init_response(auth_init_response)
	    
	    print 'Step 2: send authentication start message(including user name, password, etc.) to the server'
	    c1_nonce, auth_start_response = self._auth_start(solved_challenge, userName, password)
	    ...
	    return login_result, userName

    def _auth_init(self):
	init_msg = Message.dumps(MessageType.INIT)
	print 'init_msg: ' + init_msg
    ...
    def _auth_start(self, solved_challenge, userName, password):
	...
	#encrypted_msg_str = Crypto.asymmetric_encrypt(self.rsa_pub_key, msg_str)
	...
	print 'auth_start_response: ' + auth_start_response
	...
	try:
	    ...
	    if validate_result:
		print MSG_PROMPT + 'Online users: ' + ', '.join(list_response.userNames.split(SEPARATOR1))
		# set the client information in self.online_list
		parsed_list_response = list_response.userNames.split(SEPARATOR1)
		for user in parsed_list_response:
		    if user != self.userName and user not in self.online_list:
	...
	try:
	    ...
	    if receiver_name == self.userName:
	...
	self._send_sym_encrypted_msg_to_server(MessageType.GET_USER_INFO, userName)
	validate_result, user_info_obj = self._recv_sym_encrypted_msg_from_server()
	if validate_result:
	    # print target_address
	    user_info = self.online_list[userName]
	...
	if not Crypto.verify_signature(self.server_pub_key, ticket, ticket_signature):
	    return
	src_userName, sec_session_key, timestamp_to_expire = ticket.split(SEPARATOR1)
	if src_userName != conn_start_msg.userName or float(timestamp_to_expire) < time.time():
	    return
	...
	self.online_list[conn_start_msg.userName] = src_user_info
	...
	user_info = self.online_list[conn_back_msg.userName]
	...
	if str(decrypted_c3_nonce) == str(user_info.c3_nonce):
	    # print 'Successfully connected to the user <' + conn_back_msg.userName + '>'
	...
	user_info = self.online_list[conn_end_msg.userName]
	...
	userName = text_msg.userName
	if userName in self.online_list and self.online_list[userName].connected:
	    user_info = self.online_list[userName]
		...
		# print '\n' + MSG_PROMPT + '<From ' + ip + ':' + str(port) + ':' + userName + ">: " + decrypted_msg
		print '\n' + userName + MSG_PROMPT + decrypted_msg
		print self.userName + CMD_PROMPT,

    def _handle_disconn_msg(self, disconn_msg):
	userName = disconn_msg.userName
	if userName in self.online_list:
	    del self.online_list[userName]

    # --------------------------- logout the user and exit the program ------------------------- #
    def do_logout(self, arg):
	try:
	    if self._logout_from_server():
		print '###### User <' + self.userName + '> successfully logout.'
	...
	for userName, user_info in self.online_list.iteritems():
	    if user_info.connected:
		print '###### Disconnect to the user <' + userName + '>'
		disconn_msg = DisconnMsg(self.userName, time.time())
	...
	print '|| send <userName> <message>: send message to another online user  ||'
...
if __name__ == '__main__':
    ...
    server_pub_key = config.get('server_info', 'public_key')
